...
$tenantInfrastructureProject = "..\..\src\$projectName.TenantInfrastructure\TenantInfrastructure\TenantInfrastructure.csproj"
$tenantInfrastructureContext = "MasterDbContext"
$tenantInfrastructureMigrationPath = "MasterDb/Migrations"
...
if ($targetContext -eq 'All' -or $targetContext -eq $tenantInfrastructureContext) {
    $tenantProjectPath = (Get-Item -Path $tenantInfrastructureProject -Verbose).FullName

    if ((Test-Path $tenantProjectPath) -eq $true) {
        Write-Host "Generate migration for tenant infrastructure context:" $tenantInfrastructureContext ", for project path - " $tenantProjectPath
        dotnet ef migrations add $migration -c $tenantInfrastructureContext -o $tenantInfrastructureMigrationPath -p $tenantProjectPath -s $tenantProjectPath
    }
}
