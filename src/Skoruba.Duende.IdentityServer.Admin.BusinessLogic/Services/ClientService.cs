...
namespace Skoruba.Duende.IdentityServer.Admin.BusinessLogic.Services
{
    public class ClientService : IClientService
    {
        ...
        private static void ValidateClientUrls(ClientDto client)
        {
            var invalidUrls = new List<string>();

            invalidUrls.AddRange(GetInvalidUrls(client.RedirectUris));
            invalidUrls.AddRange(GetInvalidUrls(client.PostLogoutRedirectUris));
            invalidUrls.AddRange(GetInvalidUrls(client.AllowedCorsOrigins));

            AddIfInvalid(invalidUrls, client.ClientUri);
            AddIfInvalid(invalidUrls, client.FrontChannelLogoutUri);
            AddIfInvalid(invalidUrls, client.BackChannelLogoutUri);
            AddIfInvalid(invalidUrls, client.InitiateLoginUri);
            ...
            var message = $"Loopback URLs using 127.0.0.1 or insecure http://localhost are not allowed: {string.Join(", ", invalidUrls.Distinct(StringComparer.OrdinalIgnoreCase))}";
            throw new UserFriendlyViewException(message, "Error", client);
        }

        private static IEnumerable<string> GetInvalidUrls(IEnumerable<string> urls)
        {
            return urls?
                .Where(IsInvalidClientUrl)
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToList()
                ?? new List<string>();
        }

        private static void AddIfInvalid(ICollection<string> invalidUrls, string url)
        {
            if (IsInvalidClientUrl(url))
            {
                invalidUrls.Add(url.Trim());
            }
        }

        private static bool IsInvalidClientUrl(string url)
        {
        ...
        private static List<ClientTenantRedirectUri> MapTenantRedirectPairs(IEnumerable<ClientTenantRedirectPairDto> pairs)
        {
            return ClientTenantRedirectPairsHelper.NormalizePairs(pairs)
                .Select(x => new ClientTenantRedirectUri
                {
        ...
        private static void NormalizeClientRedirectMode(ClientDto client)
        {
            if (client == null)
            {
                return;
            ...
            client.TenantRedirectPairs = ClientTenantRedirectPairsHelper.NormalizePairs(client.TenantRedirectPairs);

            if (client.UseTenantRedirectPairs)
            {
            ...
            client.TenantRedirectPairs = new List<ClientTenantRedirectPairDto>();
        }

        private static bool HasTenantRedirectPairs(ClientDto client)
        {
            return ClientTenantRedirectPairsHelper.NormalizePairs(client?.TenantRedirectPairs).Count > 0;
        }
        ...
        public virtual async Task<int> AddClientAsync(ClientDto client)
        {
            ...
            NormalizeClientRedirectMode(client);
            ClientTenantRedirectPairsHelper.ApplyRedirectUris(client);
            ClientTenantRedirectPairsHelper.StripStoredProperty(client);
            ValidateClientUrls(client);
            PrepareClientTypeForNewClient(client);
            var clientEntity = client.ToEntity();
            var tenantRedirectPairs = MapTenantRedirectPairs(client.TenantRedirectPairs);

            var added = await ClientRepository.AddClientAsync(clientEntity, tenantRedirectPairs, client.UseTenantRedirectPairs);
        ...
        public virtual async Task<int> UpdateClientAsync(ClientDto client, bool updateClientClaims = false, bool updateClientProperties = false)
        {
            ...
            var originalClientEntity = await ClientRepository.GetClientAsync(client.Id);
            if (originalClientEntity == null)
            {
                throw new UserFriendlyErrorPageException(string.Format(ClientServiceResources.ClientDoesNotExist().Description, client.Id));
            }

            var originalClient = originalClientEntity.ToModel();
            var originalClientTenantRedirectPairs = await ClientRepository.GetClientTenantRedirectUrisAsync(client.Id);
            var originalClientUseTenantRedirectPairs = await ClientRepository.GetClientUseTenantRedirectPairsAsync(client.Id);
            if (originalClientTenantRedirectPairs.Count > 0)
            {
                originalClient.TenantRedirectPairs = originalClientTenantRedirectPairs
                    .Select(x => new ClientTenantRedirectPairDto
                    {
                        TenantKey = x.TenantKey,
                        SignInCallbackUrl = x.SignInCallbackUrl,
                        SignOutCallbackUrl = x.SignOutCallbackUrl,
                    ...
                    .ToList();
                ClientTenantRedirectPairsHelper.PopulateMissingPairValuesFromFlatLists(originalClient);
                ClientTenantRedirectPairsHelper.StripStoredProperty(originalClient);
                ClientTenantRedirectPairsHelper.StripPairRedirectUris(originalClient);
            }
            else
            {
                ClientTenantRedirectPairsHelper.PopulateFromStoredProperty(originalClient);
                ClientTenantRedirectPairsHelper.PopulateMissingPairValuesFromFlatLists(originalClient);
                ClientTenantRedirectPairsHelper.StripPairRedirectUris(originalClient);
            }
            originalClient.UseTenantRedirectPairs = originalClientUseTenantRedirectPairs || HasTenantRedirectPairs(originalClient);
            ...
            var updated = await ClientRepository.UpdateClientAsync(clientEntity, tenantRedirectPairs, updateClientClaims, updateClientProperties, client.UseTenantRedirectPairs);
        ...
        public virtual async Task<ClientDto> GetClientAsync(int clientId)
        {
            ...
            var tenantRedirectPairs = await ClientRepository.GetClientTenantRedirectUrisAsync(clientId);
            var useTenantRedirectPairs = await ClientRepository.GetClientUseTenantRedirectPairsAsync(clientId);
            if (tenantRedirectPairs.Count > 0)
            {
                clientDto.TenantRedirectPairs = tenantRedirectPairs
                    .Select(x => new ClientTenantRedirectPairDto
                    {
                        TenantKey = x.TenantKey,
                        SignInCallbackUrl = x.SignInCallbackUrl,
                    ...
                    .ToList();
                ClientTenantRedirectPairsHelper.PopulateMissingPairValuesFromFlatLists(clientDto);
                ClientTenantRedirectPairsHelper.StripStoredProperty(clientDto);
                ClientTenantRedirectPairsHelper.StripPairRedirectUris(clientDto);
            }
            else
            {
                ClientTenantRedirectPairsHelper.PopulateFromStoredProperty(clientDto);
                ClientTenantRedirectPairsHelper.PopulateMissingPairValuesFromFlatLists(clientDto);
                ClientTenantRedirectPairsHelper.StripPairRedirectUris(clientDto);
            }
            clientDto.UseTenantRedirectPairs = useTenantRedirectPairs || HasTenantRedirectPairs(clientDto);
        ...
        public virtual async Task<ClientsDto> GetClientsAsync(string search, int page = 1, int pageSize = 10)
        {
            ...
            clientsDto.Clients?.ForEach(ClientTenantRedirectPairsHelper.StripStoredProperty);
