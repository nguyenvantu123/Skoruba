...
namespace Skoruba.Duende.IdentityServer.Admin.EntityFramework.Identity.Repositories
{
        ...
        where TUserToken : IdentityUserToken<TKey>
    {
        ...
        protected readonly ILogger<IdentityRepository<TIdentityDbContext, TUser, TRole, TKey, TUserClaim, TUserRole, TUserLogin, TRoleClaim, TUserToken>> Logger;
        ...
        public IdentityRepository(TIdentityDbContext dbContext,
            ...
            IMapper mapper,
            ILogger<IdentityRepository<TIdentityDbContext, TUser, TRole, TKey, TUserClaim, TUserRole, TUserLogin, TRoleClaim, TUserToken>> logger)
        {
            DbContext = dbContext;
        ...
        public virtual async Task<PagedList<TUser>> GetUsersAsync(string search, int page = 1, int pageSize = 10, string tenantKey = null)
        {
            ...
            query = ApplyTenantFilter(query, tenantKey);

            var users = await query.PageBy(x => x.Id, page, pageSize).ToListAsync();
        ...
        private IQueryable<TUser> ApplyTenantFilter(IQueryable<TUser> query, string tenantKey)
        {
            ...
            return query.Where(x => EF.Property<string>(x, "TenantKey") == normalizedTenantKey);
        }
        ...
        public virtual async Task<(IdentityResult identityResult, TKey userId)> CreateUserAsync(TUser user, string password = null)
        {
            var hasPassword = !string.IsNullOrWhiteSpace(password);
            var passwordLength = password?.Length ?? 0;

            Logger.LogInformation(
                "IdentityRepository.CreateUserAsync invoked. UserName={UserName}, UserId={UserId}, HasPassword={HasPassword}, PasswordLength={PasswordLength}",
                user.UserName,
                user.Id,
                hasPassword,
                passwordLength);
