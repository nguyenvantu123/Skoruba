...
namespace Skoruba.Duende.IdentityServer.STS.Identity.Controllers
{
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        private readonly IClientTenantRedirectResolver _clientTenantRedirectResolver;
        ...
        public AccountController(
            ...
            IClientTenantRedirectResolver clientTenantRedirectResolver)
        {
            ...
            _clientTenantRedirectResolver = clientTenantRedirectResolver;
        }
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                await FirstTimeLoginClaimHelper.SetFirstTimeLoginAsync(_userManager, user, false);
                return RedirectToAction(nameof(ResetPasswordConfirmation), "Account");
            }
        ...
        private async Task<string?> ResolveTenantRedirectUrlAsync(Client? client)
        {
            ...
            var clientRedirectUrl = await _clientTenantRedirectResolver.ResolveRedirectUrlAsync(client, tenantKey, HttpContext.RequestAborted);
        ...
        private async Task<string?> ResolveTenantPostLogoutRedirectUrlAsync(Client? client)
        {
            var tenantKey = _tenantAccessor.Current?.TenantKey;
            if (string.IsNullOrWhiteSpace(tenantKey)) return null;

            var clientPostLogoutRedirectUrl = await _clientTenantRedirectResolver.ResolvePostLogoutRedirectUrlAsync(client, tenantKey, HttpContext.RequestAborted);
            return string.IsNullOrWhiteSpace(clientPostLogoutRedirectUrl)
                ? null
                : clientPostLogoutRedirectUrl;
        }
        ...
        private async Task<LoggedOutViewModel> BuildLoggedOutViewModelAsync(string logoutId)
        {
            ...
            if (string.IsNullOrWhiteSpace(postLogoutRedirectUri) &&
                !string.IsNullOrWhiteSpace(logout?.ClientId))
            {
                var client = await _clientStore.FindEnabledClientByIdAsync(logout.ClientId);
                postLogoutRedirectUri = await ResolveTenantPostLogoutRedirectUrlAsync(client);
            }
        ...
        private async Task EnsureLoginAllowedAsync(TUser user)
        {
            ...
            var adminAccess = await GetAdminAccessAsync(user);

            if (tenant == null)
            {
                if (!adminAccess.IsSuperAdmin)
                    throw new SecurityException("User is not allowed to login on global host.");
        ...
        private async Task<IActionResult?> TryTenantAdminRedirectViewAsync(TUser user, AuthorizationRequest? context)
        {
            ...
            var adminAccess = await GetAdminAccessAsync(user);
            if (!adminAccess.IsTenantAdmin) return null;

            var redirectUrl = await ResolveTenantRedirectUrlAsync(context.Client);
        ...
        private async Task<(bool IsSuperAdmin, bool IsTenantAdmin)> GetAdminAccessAsync(TUser user)
        {
