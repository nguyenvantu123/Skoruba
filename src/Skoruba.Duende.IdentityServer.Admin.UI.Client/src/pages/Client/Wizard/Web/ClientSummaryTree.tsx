...
const ClientWebSummaryTree = () => {
  ...
  const clientInfoRows: DisplayLine[] = [
    ...
    />,
    <SummaryRow
      key="useTenantRedirectPairs"
      label={t("Client.Label.UseTenantRedirectPairs_Label")}
      value={yesNo(formData.useTenantRedirectPairs)}
    />,
  ...
  const oauthClientData: OAuthNode = {
    ...
    children: [
      ...(excludeOptions?.uris
        ? []
        : [
            ...(formData.useTenantRedirectPairs
              ? [
                  {
                    step: 2,
                    name: t("Client.TenantRedirects.Title"),
                    type: OAuthNodeType.URI,
                    value: (formData.tenantRedirectPairs ?? [])
                      .filter(
                        (pair) =>
                      ...
                      .map((pair) => (
                        <SummaryRow
                          key={pair.tenantKey}
                          label={pair.tenantKey}
                          value={[
                            pair.signInCallbackUrl &&
                              `${t("Client.TenantRedirects.RedirectUrlLabel")}: ${pair.signInCallbackUrl}`,
                            pair.signOutCallbackUrl &&
                              `${t("Client.TenantRedirects.PostLogoutRedirectUrlLabel")}: ${pair.signOutCallbackUrl}`,
                            pair.corsOrigin &&
                              `${t("Client.TenantRedirects.CorsOriginLabel")}: ${pair.corsOrigin}`,
                          ]
                            .filter(Boolean)
                            .join(" | ")}
                        />
              ...
              : [
                  {
                    step: 2,
                    name: t("Client.TenantRedirects.RedirectUrlLabel"),
                    type: OAuthNodeType.URI,
                    value: formData.signInCallbackUrl || "-",
                  },
                  {
                    step: 2,
                    name: t("Client.TenantRedirects.PostLogoutRedirectUrlLabel"),
                    type: OAuthNodeType.URI,
                    value: formData.signOutCallbackUrl || "-",
                  },
                  {
                    step: 2,
                    name: t("Client.TenantRedirects.CorsOriginLabel"),
                    type: OAuthNodeType.URI,
