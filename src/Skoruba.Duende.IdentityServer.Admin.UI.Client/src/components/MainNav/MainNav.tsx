...
const platformItems: NavItem[] = [
  {
    translationKey: "Home.Tenants",
    href: TenantsUrl,
    activeBasePaths: [
      TenantsUrl,
      getBasePath(TenantEditUrl),
      getBasePath(TenantCreateUrl),
    ],
    icon: Building2,
    kind: "management",
  },
...
export function MainNav() {
  ...
  const visibleIdentityItems = isSuperAdmin ? identityItems : [];
  const visiblePlatformItems = isSuperAdmin ? platformItems : [];
        ...
        {visibleIdentityItems.length > 0 && (
          ...
          >
            <div className="grid min-w-[360px] grid-cols-1 gap-2">
              {visibleIdentityItems.map((it) => renderDropdownItem(it))}
            </div>
          </NavDropdown>
        )}
        ...
        {visiblePlatformItems.length > 0 && (
          ...
          >
            <div className="grid min-w-[320px] grid-cols-1 gap-2">
              {visiblePlatformItems.map((it) => renderDropdownItem(it))}
            </div>
          </NavDropdown>
        )}
...
function MobileNav() {
  ...
  const groups = [
    ...
    },
    {
      ...
      icon: <Server className="h-4 w-4" />,
      items: isSuperAdmin ? platformItems : [],
    },
